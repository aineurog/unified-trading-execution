...
from unified_trading_execution.types.instrument import Instrument, InstrumentSpec, _with_broker_override
...
def test_symbol_must_not_be_empty():
    with pytest.raises(ValueError, match="symbol must be non-empty and uppercase"):
        Instrument(
            symbol="", quote_currency="USDT", asset_class=AssetClass.SPOT,
            exchange=None, currency=None, expiry=None, strike=None,
            option_right=None, multiplier=None,
        )
...
def test_option_requires_expiry():
    with pytest.raises(ValueError, match="expiry is required for OPTION"):
        Instrument(
            symbol="AAPL", quote_currency=None, asset_class=AssetClass.OPTION,
            exchange="SMART", currency="USD", expiry=None,
            strike=Decimal("200"), option_right=OptionRight.CALL, multiplier=100,
        )
...
def test_option_requires_strike():
    with pytest.raises(ValueError, match="strike is required for OPTION"):
        Instrument(
            symbol="AAPL", quote_currency=None, asset_class=AssetClass.OPTION,
            exchange="SMART", currency="USD", expiry=date(2026, 12, 18),
            strike=None, option_right=OptionRight.CALL, multiplier=100,
        )
...
def test_option_requires_option_right():
    with pytest.raises(ValueError, match="option_right is required for OPTION"):
        Instrument(
            symbol="AAPL", quote_currency=None, asset_class=AssetClass.OPTION,
            exchange="SMART", currency="USD", expiry=date(2026, 12, 18),
            strike=Decimal("200"), option_right=None, multiplier=100,
        )
...
def test_futures_requires_multiplier():
    with pytest.raises(ValueError, match="multiplier is required"):
        Instrument(
            symbol="ES", quote_currency="USD", asset_class=AssetClass.FUTURES,
            exchange="CME", currency="USD", expiry=date(2026, 12, 18),
...
def test_option_requires_multiplier():
    with pytest.raises(ValueError, match="multiplier is required"):
        Instrument(
            symbol="AAPL", quote_currency=None, asset_class=AssetClass.OPTION,
            exchange="SMART", currency="USD", expiry=date(2026, 12, 18),
            strike=Decimal("200"), option_right=OptionRight.CALL, multiplier=None,
        )
...
def test_stock_str_raises():
    inst = Instrument(
        symbol="AAPL", quote_currency=None, asset_class=AssetClass.STOCK,
        exchange=None, currency="USD", expiry=None, strike=None,
        option_right=None, multiplier=None,
    )
    with pytest.raises(ValueError, match="cannot be represented as a shorthand string"):
        str(inst)
