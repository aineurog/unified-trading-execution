...
def test_fill_event_constructs():
    ...
    e = FillEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1", fill=fill,
    )
    assert e.fill == fill
...
def test_position_update_event_constructs():
    ...
    e = PositionUpdateEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1", position=pos,
    )
...
def test_balance_update_event_constructs():
    ...
    e = BalanceUpdateEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1", balance=bal,
    )
...
def test_connection_state_event_constructs():
    e = ConnectionStateEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None, connected=True,
    )
...
def test_order_placed_event_constructs():
    ...
    e = OrderPlacedEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1", order=order,
    )
...
def test_order_modified_event_constructs():
    ...
    e = OrderModifiedEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1",
        order=updated, previous=prev,
    )
...
def test_order_cancelled_event_constructs():
    ...
    e = OrderCancelledEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id="corr-1",
        client_order_id="abc", instrument=inst,
    )
...
def test_reconciliation_complete_event_clean():
    e = ReconciliationCompleteEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None, mismatches=(),
    )
    assert e.mismatches == ()
...
def test_reconciliation_complete_event_with_mismatches():
    ...
    e = ReconciliationCompleteEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None, mismatches=(m,),
    )
...
def test_halt_entered_event_instrument_scope():
    ...
    e = HaltEnteredEvent(
        ...
        scope="instrument", instrument=inst,
...
def test_halt_cleared_event_automatic():
    e = HaltClearedEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None,
        scope="instrument", instrument=make_instrument(),
...
def test_halt_cleared_event_manual():
    e = HaltClearedEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None,
        scope="account", instrument=None, cleared_by="manual",
    )
...
def test_reconciliation_mismatch_all_types():
    for mtype in ("position_quantity", "balance", "orphan_on_platform",
...
def test_halt_event_cleared():
    e = HaltEvent(
        event_id="evt-1", timestamp=NOW, adapter_name="bybit",
        account_id="acct-1", correlation_id=None,
        action="cleared", scope="instrument",
        instrument=make_instrument(),
        reason="",
...
class TestEventBus:
    def test_subscribe_and_receive(self):
        ...
        evt = Event(event_id="e1", timestamp=NOW, adapter_name="a",
                     account_id="acct", correlation_id=None)
        bus.publish(evt)
    ...
    def test_publish_to_multiple_subscribers(self):
        ...
        bus.publish(Event(event_id="e1", timestamp=NOW, adapter_name="a",
                           account_id="acct", correlation_id=None))
        assert received == ["a", "b"]
    ...
    def test_supertype_subscription_receives_subtypes(self):
        ...
        bus.publish(FillEvent(
            event_id="e1", timestamp=NOW, adapter_name="a",
            account_id="acct", correlation_id=None,
            fill=make_fill_record(),
        ))
        bus.publish(ConnectionStateEvent(
            event_id="e2", timestamp=NOW, adapter_name="a",
            account_id="acct", correlation_id=None, connected=True,
        ))
